...
class DispMasc(var id:Int, var nombre:String,var tipo:String,
    ...
    override fun getDispMascPorId(id: Int): Call<List<DataDispMasc>> {
        val sql = "select d.id,d.nombre,t.nombre_t 'tipo',t.info_extra 't_info'," +
                "d.lavados,d.duracion,d.stock,d.comentario " +
                "from disp_masc d " +
                "join (tipos_masc t) " +
    ...
    override fun updateDispMasc(
        ...
        val sql  = " update disp_masc set nombre = '$nombre', " +
                "lavados = $lavados , duracion = $duracion , stock = $stock , "+
                " comentario = '$comentario' where id = $id; "
        return  c.execute(sql)
    }

    override fun deleteDispMasc(id: Int): Call<DataCodigoError> {
        val sql  = "delete from disp_masc where id = $id;"
        return  c.execute(sql)
    }
